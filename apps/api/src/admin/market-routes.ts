...
const patchSchema = z.object({ mode: z.enum(["REAL", "MANUAL"]).optional(), tradingFee: decimal.optional(), spread: decimal.optional(), slippage: decimal.optional(), volatility: decimal.optional(), orderBookLevels: z.number().int().min(20).max(50).optional(), simulationPaused: z.boolean().optional() });
const manualPriceSchema = z.object({ referencePrice: decimal });
...
export const registerAdminMarketRoutes = (app: Express, { auth, users, pool, onMarketChanged }: Dependencies) => {
  ...
  app.patch("/api/admin/market/config", authenticate, requireAdministrator, asyncRoute(async (request, response) => {
    ...
    if (!body.success || Object.keys(body.data).length === 0) { response.status(400).json({ error: { code: "VALIDATION_ERROR" } }); return; }
    ...
    try {
      ...
      await client.query("INSERT INTO admin_audit_events (admin_user_id, action, entity_type, metadata, reason) VALUES ($1, 'MARKET_CONFIGURATION_CHANGED', 'MARKET_CONFIGURATION', $2::jsonb, 'ADMINISTRATOR_ACTION')", [administrator.userId, JSON.stringify(body.data)]);
  ...
  app.put("/api/admin/market/manual-prices/:asset", authenticate, requireAdministrator, asyncRoute(async (request, response) => {
    ...
    try {
      ...
      await client.query("INSERT INTO admin_audit_events (admin_user_id, action, entity_type, metadata, reason) VALUES ($1, 'MANUAL_MARKET_PRICE_SET', 'MANUAL_MARKET_PRICE', $2::jsonb, 'ADMINISTRATOR_ACTION')", [administrator.userId, JSON.stringify({ asset, referencePrice: body.data.referencePrice })]);
      await client.query("COMMIT");
      onMarketChanged?.();
